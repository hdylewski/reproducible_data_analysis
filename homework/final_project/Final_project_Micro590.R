  ...
  #______________________ Dashboard content______________________
            tabItem(tabName = "dashboard",
                    ...
                    ### Quick analysis panel
                        column(3, offset = 1,
                               textInput("average_1","Sample Name 1", "i.e. CB-A"),
                               textInput("average_2","Sample Name 2", "i.e. CB-D"),
                               textInput("average_3","Sample Name 3", "i.e. Coculture"),
                               submitButton("Update")
                        ),
                        
                        ### Average Table Outputs
                        column(7, offset = 1,
   ...
   observe({
    ...
    output$csv.plot <- renderPlot({
      data <- sample_data()
        ...
        ### axis labels, and overall color scheme. All color schemes are supposed to be color blind friendly
        long_data_final <- rename(long_data, 'Sample' = contains('Sample'))

        
        if(input$user_plot_type == "Line"){
          p <- ggplot(long_data_final, aes(x = reorder(Time_Points, sort(as.numeric(Time_Points))), y = Number, color = Sample, group = Sample))+
           ...
           } else if(input$user_plot_type == "Box Plot"){
             
             p <-ggboxplot(long_data_final, x = "Time_Points", y = "Number", color = "Sample") +
               xlab(input$user_x_axis_label) + ### x-axis label text
               ylab(input$user_y_axis_label) + ### y-axis label text
               ggtitle(input$user_graph_title) + ## Graph title text
               theme(plot.title = element_text(color = input$user_title_color, ### Graph Title format
               ...
               scale_color_viridis(discrete = TRUE, option = input$user_color_palette) ### sets a color blind friendly color pallette
                   ...
                   ### add PWC and outliers to a box plot if option was selected
                  if (input$user_stats_graph == "Pairwise" && !is.null(stat_results$pwc) ){
                    pwc_plot <- stat_results$pwc %>% add_xy_position(x = "Time_Points")
                      p <- p + stat_pvalue_manual(pwc_plot, hide.ns = TRUE) +
                        labs(
                          caption = get_pwc_label(pwc_plot),
                          outlier.shape = 8, position = position_dodge(1))
                  }
             }
        # graph
        p
